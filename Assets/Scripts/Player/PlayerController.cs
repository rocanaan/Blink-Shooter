...
public class PlayerController : MonoBehaviour {
		...
		if (!GameController.isGameOver ()) {
			if (isDead && Time.time >= nextRespawnTime && respawnAllowed) {
				...
				if (gameController.gameMode == GameController.GameMode.Boss) {
                    ...
                    Debug.Log("Respawn position: " + randomVector);
					transform.position = new Vector3 (randomVector.x, randomVector.y, transform.parent.transform.position.z);
    ...
    //************************************************************************************
    // USING THE FireShot SCRIPT INSTEAD FROM NOW ON
    //************************************************************************************
...
//			// Get rotation input
			...
			if (controllerName == "Keyboard") {
				...
				float aimVertical = Input.GetAxis ("AimVertical" + controllerName);
